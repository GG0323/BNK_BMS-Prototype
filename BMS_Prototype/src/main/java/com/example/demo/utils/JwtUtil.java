...
public class JwtUtil {
	...
	public String refleshToken(String username, Role role) {
		return Jwts.builder()
				.subject(username)
				.claim("role", role)
				.issuedAt(new Date())
	...
	public String updateAccessToken(String token) {
		...
		return accessToken(data.getSubject(), data.get("role", Role.class));
	}
	
	public String updateRefleshToken(String token) {
		Claims data = parseToken(token);
		return refleshToken(data.getSubject(), data.get("role", Role.class));
	}
